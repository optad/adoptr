...
There are two fundamental ways of scoring a two-stage design: First, one
may assess the performance before observing any data, i.e., at the
...
![](conditional-scores_files/figure-html/define-design-1.png)

In adoptr, scores are instances of their respective score class. The
most important ones are: `ConditionalScore`, `UnconditionalScore`, and
`IntegralScore`. An object of class `ConditionalScore` can evaluate a
design for a particular stage-one outcome. A `ConditionalScore` is a
function $`s(\mathcal{D}, x_1)`$ evaluating a design $`\mathcal{D}`$ at
a stage-one outcome $`X_1 = x_1`$. Some conditional scores might depend
on the data distribution (conditional power) others do not (conditional
sample size). Conditional score evaluation is completely vectorized:
...
![](conditional-scores_files/figure-html/unnamed-chunk-3-1.png)

Any conditional score can be integrated with respect to a prior and data
distribution to obtain an unconditional score. Note that for conditional
scores which depend on a specification of these distributions (e.g.,
conditional power) these arguments must be consistent! The resulting
score is of class `IntegralScore`, a specific subclass of
`UnconditionalScore` and evaluates designs independent of a particular
$`x_1`$, i.e., unconditionally
...
the continuation area only. E.g.,

``` r

cp >= 0.7
#> -Pr[x2>=c2(x1)|x1] (x1) <= -0.7 for x1 in [c1f,c1e]
```
...
#> Pr[x2>=c2(x1)|x1]  - Pr[x2>=c2(x1)|x1] (x1) <= 0 for x1 in [c1f,c1e]
```
